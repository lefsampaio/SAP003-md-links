...
const errorLinks = [
  { href: "https://boadsad.com/", text: "erro", status: 404 },
  { href: "https://iuhihoooooooo.com/", text: "erro", status: 404 }
];
...
describe("mdLinks", () => {
  ...
  it("Should return 3 links with status", (done) => {
    mdLinks("./lib/__test__/text.md", {validate: true}).then(result => {
  ...
  it("Should return 3 links with 'not found' status", (done) => {
    mdLinks("./lib/__test__/erro404.md", {validate: true}).then(result => {
      expect(result).toEqual(errorLinks);
      done();
    });
  });
});
